...
def create_buses_links_lines_map(buses_csv: str, links_csv: str, lines_csv: str, geojson_file: str, output_file: str = "buses_links_lines_map.html", args: dict = None):
    ...
    # Lines plotten mit s_max_pu-Farbskala
    for _, row in lines_df.iterrows():
        try:
            point0 = bus_lookup.loc[row['bus0'], 'geometry']
            point1 = bus_lookup.loc[row['bus1'], 'geometry']
    ...
    <div style="position: fixed; bottom: 30px; left: 30px; width: 200px; height: auto;
         background-color: white; border:22px solid grey; z-index:9999; font-size:14px;
         padding: 10px;">
    <b>Carrier Legende</b><br>
    """
    for carrier, color in carrier_color_map.items():
    ...
    # Karte speichern
    m.save(output_file)
    print(f"✅ Interaktive Karte mit Bussen, Links und Lines gespeichert unter: {output_file}")
